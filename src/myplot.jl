...
function plotgraph(A,xy) 
    ...
    plot(px,py,markersize=6, linecolor=1, linealpha=0.8, linewidth=0.7,
    markercolor=colorant"black", markerstrokecolor=colorant"white", 
    framestyle=:none, legend=false, marker=:dot)
end 
...
function showvector(x)
    ...
    x .*= sign(x[end])
    p = plotgraph(A,xy)
    scatter!(p, xy[1,vperm], xy[2,vperm], marker_z=x[vperm], markerstrokecolor=:black,
             alpha=1.0, markerstrokewidth=0, markersize=6, border=:none)
    p2 = scatter(-(1:length(x)), (0.5*x[vperm]), label="", border=:none, markersize=6,
                 markerstrokewidth=0.0, marker_z=x[vperm], colorbar=false)
...
function lrsdpobjplot(
    ...
    Gvol = 0
    for mu in muvals
        path2res = datafolder*filename*"/"*filename*"-$Ktol-$Ptol-$k-$mu-$(init_type).mat"
        if isfile(path2res)
            @show mu
    ...
    plot!(xvals * Gvol, yvals ./ 2, linewidth=2, xscale=:log10, yscale=:log10, xlabel=xlabel, ylabel=ylabel, legend=false, label=label, marker=:d)
    return cnt
end
