...
function counts2xy_(counts::Dict{S,T}, xsize, ysize, x0, y0) where {S,T}
    ...
    for (idx, cnt) in counts
        k += 1
        xx, yy = Hexagons.center(
            HexagonOffsetOddR(idx[1], idx[2]),
            xsize,
            ysize,
            x0,
            y0,
...
function fit(
    ...
    (length(x) == length(y)) ||
        throw(ArgumentError("data vectors must be commensurate"))
    (xyweights == nothing) ||
        (length(xyweights) == length(x)) ||
        throw(ArgumentError("data and weight vectors must be commensurate"))
